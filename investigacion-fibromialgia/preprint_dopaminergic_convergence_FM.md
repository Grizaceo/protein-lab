...
**Preprint — Draft v2.3 — August 2026**
...
**UKB Causal Gene Set.** The 10 proteins validated as causal for chronic pain / chronic widespread pain in UKB Olink proteomics (Li ZY et al., 2025; Chen et al., 2024): *CA14*, *LEP*, *TNFRSF1B*, *CD74*, *COL18A1*, *BTN2A1*, *TNFRSF4*, *CD302*, *TNFRSF9*, *TNF*.
...
### 3.4 UK Biobank Causal Proteomics: CA14 Elevated, Immune-Signaling Genes Downregulated in FM PBMCs

To connect population-scale causal proteomics with our transcriptomic data, we tested the UKB causal genes (Li ZY et al., 2025; Chen et al., 2024) in GSE221921. Nine of ten are measurable (RARRES2/TNF and LEP absent from the matrix).
...
**CA14 closes a causal triangle.** CA14 — the **top-ranking causal protein for chronic widespread pain** in the UKB preprint (MR + colocalization; Chen et al., 2024) — is the **only UKB causal gene that is upregulated in FM PBMCs** (FC = 2.29, p = 0.0003, d = +0.41). CA14 encodes carbonic anhydrase XIV, which regulates pH and is involved in neuronal signal transmission via zinc ion binding and carbonate dehydratase activity. It has a **known inhibitor, sulthiame (CHEMBL328560)**, an approved anti-epileptic drug. The triangle: (1) causal in plasma proteomics (UKB, MR+coloc); (2) elevated in PBMC mRNA (our data); (3) druggable by an approved compound. This makes CA14 the strongest cross-level candidate in the entire investigation.
...
2. **CA14 — the top UKB causal protein for chronic widespread pain — is elevated** (FC = 2.29), while **five UKB causal immune-signaling genes are downregulated** (TNFRSF1B, CD74, COL18A1, BTN2A1, TNFRSF4).
...
### 4.2 CA14: A Cross-Level Causal Candidate With an Existing Drug

CA14 emerges as the most actionable candidate in this investigation:

| Layer | Evidence | Source |
|-------|----------|--------|
| 1. Plasma protein | Causal in CWP/FM (top-ranking, MR + colocalization) | UKB, Chen et al. 2024 (29,254 participants) |
| 2. PBMC mRNA | ↑ in FM (FC = 2.29, p = 0.0003, d = +0.41) | GSE221921 (this study) |
| 3. Pharmacology | Known inhibitor: sulthiame (CHEMBL328560), approved anti-epileptic | Chen et al. 2024; ChEMBL |
...
**Druggability caveat.** Sulthiame inhibits multiple carbonic anhydrases, not only CA14; any repurposing hypothesis requires isoform selectivity assessment and cannot be inferred from the present transcriptomic analysis alone.
...
Chen, L., et al. (2024). Plasma proteomics of chronic widespread pain and fibromyalgia in UK Biobank. *medRxiv*. doi: 10.1101/2024.10.29.24316353. (EuropePMC PPR932603.)
