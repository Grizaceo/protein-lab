...
**Figure 1. Sensitivity analysis: nested models on log₂(FPKM+1).** M1 (group only), M2 (+ sex), M3 (+ cell composition, all 12 fractions). COL9A1 and PTN (blue) are the only genes that survive all three models. The opioid/tachykinin axis (grey) collapses under the *full* composition adjustment — but this collapse is collinearity-driven (max VIF 27.8); under an orthogonal adjustment that drops high-VIF fractions (Table 2, "Orthogonal-adj. p") TACR1/OPRM1/TAC1 regain significance. The dashed line marks p = 0.05.
