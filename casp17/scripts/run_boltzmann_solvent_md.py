...
def compute_electrostatic_potential_light(grid_pts, rna_coords, rna_charges):
    """
    Ultra-light memory screened electrostatic potential calculation.
    """
    kappa = 0.1
    dielectric = 78.5
    ke = 332.0637
...
def generate_boltzmann_solvent_ensemble(target_name, group_id, num_models=10, is_high_salt=False):
    ...
    print(f"[{target_name}] Running Boltzmann Ensemble Sampler for {num_models} models (Group: {group_id})...")
    ...
    for m_idx in range(1, num_models + 1):
        ...
        kB_T = 0.593
        water_weights = np.exp(-1.0 * potentials / kB_T)
        ...
        if num_k_ions > 0 and len(candidate_grid) > num_k_ions:
            k_weights = np.exp(-2.0 * potentials / kB_T)
        ...
        with open(filepath, 'w') as f:
            ...
            f.write(f"METHOD Boltzmann-Weighted Thermodynamic Solvent Sampler (Protein-Lab MD Pipeline)\n")
    ...
    print(f"[{target_name}] Boltzmann Ensemble Package created: {tarball_name}")
